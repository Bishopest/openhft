...
public class GroupedSingleOrderQuoter : IQuoter
{
    ...
    public GroupedSingleOrderQuoter(
            ILogger logger,
            Side side,
    ...
    public async Task UpdateQuoteAsync(Quote newQuote, bool isPostOnly, CancellationToken cancellationToken = default)
    {
        ...
        if (_groupingTickMultiple == null)
        {
            lock (_groupingLock)
            {
                ...
                if (_groupingTickMultiple == null)
                {
                    _groupingTickMultiple = CalculateGroupingMultiple(quoteP);
                    _logger.LogInformationWithCaller($"Calculated {_instrument.Symbol} grouping multiple {_groupingTickMultiple} ticks from side {_side}.");
                    return;
                }
            }
        }
        ...
        var groupingSizeInTicks = _instrument.TickSize.ToTicks() * _groupingTickMultiple.Value;

        // 3. Group the prices.
        var groupedTicks = _side == Side.Buy ? quoteP.ToTicks() / groupingSizeInTicks * groupingSizeInTicks : (quoteP.ToTicks() + groupingSizeInTicks - 1) / groupingSizeInTicks * groupingSizeInTicks;
        Quote groupedQuote = new Quote(Price.FromTicks(groupedTicks), newQuote.Size);

        try
        {
            LatestQuote = groupedQuote;
            IOrder? currentOrder;
            lock (_stateLock)
            {
            ...
            if (currentOrder is null)
            {
                ...
                await StartNewQuoteAsync(groupedQuote, isPostOnly, cancellationToken).ConfigureAwait(false);
            }
            else
            {
