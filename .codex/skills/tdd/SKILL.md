...
name: tdd
description: >
  Guided test-driven development workflow for .NET 10 using NUnit,
...
- **NUnit** -- Test framework with `[Test]`, `[TestCase]`, and fixture lifecycle attributes
...
1. Create test project with NUnit, NUnit3TestAdapter, and required packages
2. Configure `WebApplicationFactory` with Testcontainers for the real database
3. Add `[OneTimeSetUp]`/`[OneTimeTearDown]` fixture setup to avoid spinning up containers per test
