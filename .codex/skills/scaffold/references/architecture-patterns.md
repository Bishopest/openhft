...
public sealed class ApiFixture : WebApplicationFactory<Program>
{
    ...
    [OneTimeSetUp]
    public async Task OneTimeSetUp() { await _postgres.StartAsync(); /* apply migrations */ }

    [OneTimeTearDown]
    public async Task OneTimeTearDown() { await _postgres.DisposeAsync(); await base.DisposeAsync(); }
}
...
public sealed class CreateOrderTests
{
    ...
    public async Task CreateOrder_ValidRequest_Returns201()
    {
        ...
        Assert.That(result.GetProperty("id").GetGuid(), Is.Not.EqualTo(Guid.Empty));
    }
